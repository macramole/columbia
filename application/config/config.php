...
|--------------------------------------------------------------------------
|
| Language for admin pages. (added by Pârleboo)
|
*/
...
|
*/
$config['magico_enable_facebook'] = '587706054578576'; //false o app id
$config['magico_facebook_page'] = '273811732717797'; //id de la page
...
|--------------------------------------------------------------------------
|
| This is used by the mail function now, but should be used in other parts of the code in the future
|
| (added by Pârleboo)
|
*/
$config['magico_sitename'] = "Fundación Columbia";
...
$config['magico_nav'] = array(
	...
	'Novedad' => array(),
	'Actividad' => array(),
	'ActividadEspecial' => array('title' => 'Actividad Especial'),
	'Publicacion' => array(),
	'Docente' => array(),
	'Disciplina' => array(),
	'Sponsor' => array(),
...
$config['magico_customList'][] = array(
	'title' => 'Alumnos inscriptos - Regulares',
	'sqlList' => "
		SELECT
	...
	'sqlAction' => "
		SELECT
			...
			Email,
			Celular,
			IF(aad.activo,'Si','No') AS Pago,
			DATE_FORMAT(aad.fechaReserva,'%d/%m/%Y') AS 'Fecha Reserva'
		FROM
			`alumnos` a
		INNER JOIN
...
$config['magico_customList'][] = array(
	'title' => 'Alumnos inscriptos - Especiales',
	'sqlList' => "
		SELECT
			id AS idDisciplinaActividadEspecial,
			title AS Actividad,
					...
					(
						SELECT
							CONCAT (
								...
								WHERE
									idDisciplinaActividadEspecial = dae.id AND
									vacante = 1 ), ' / ',
									dae.vacantes, ' (',
								( SELECT
									COUNT(*)
	...
	'sqlAction' => "
		SELECT
			...
			Email,
			Celular,
			IF(aae.activo,'Si','No') AS Pago,
			IF(aae.pagoVacante,'Si','No') AS 'Solo Vacante',
			IF(aae.pagoVacante,'N/A',DATE_FORMAT(aae.fechaHasta,'%d/%m/%Y')) AS 'Fecha Vencimiento',
			DATE_FORMAT(fechaInscripcion,'%d/%m/%Y') AS 'Fecha de Inscripcion',
			MP_external_reference AS 'ID MercadoPago',
			MP_status AS 'Estado MercadoPago'
		FROM
			`alumnos` a
		INNER JOIN
			...
			aae.`idAlumno` = a.`id`
		WHERE
			aae.`idDisciplinaActividadEspecial` = {1} AND
			aae.vacante = 1
		ORDER BY
			aae.activo DESC, aae.`pagoVacante` DESC
	",
	'actionName' => 'Ver lista de alumnos',
	'actionParamField' => 'idDisciplinaActividadEspecial',
	'hiddenFields' => array('idDisciplinaActividadEspecial')
);
